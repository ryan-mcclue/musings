...
if iterating over an array where the index is being incremented not in a loop, i.e. is unbounded (e.g. every function call increment index), worthwhile to do bounds checking or some assert
...
Good to print out status of completion for threading? Could be just counting number of chunks retired?
...
Now in a while loop that runs for as many chunks as we have, we call the `render_chunk` function and pass in the WorkQueue
The `render_chunk` function will increment the next_work_order_index 
